...
use cortex_m::peripheral::{syst::SystClkSource, SYST, SCB, syst::SystClkSource::Core};
//use stm32f0::stm32f0x2::Peripherals;
use stm32f0xx_hal::stm32::{Peripherals, GPIOA, GPIOB};
use stm32f0xx_hal::rcc::{RccExt, Rcc};
...
impl<'a, T: USBHardware> USB<'a, T> {
    ...
    fn handle_device_out_transfer(&mut self, transaction: &Transaction) {
        ...
        let mut complete_command_packet: [u8; 64] = [0;64]; 
        for x in (0..64).step_by(2) {
            let current_read = self.pma.read(transaction.endpoint, x);
            complete_command_packet[x as usize] = (current_read & 0xff) as u8;
            complete_command_packet[(x+1) as usize] = ((current_read & 0xff00) >> 8) as u8;
        } 
        let mut rest_of_packet: [u8;60] = [0;60];
        rest_of_packet.copy_from_slice(&complete_command_packet[3..63]); 

        let command_packet = CommandPacket::from((
            complete_command_packet[0],
            complete_command_packet[1],
            complete_command_packet[2],
            rest_of_packet, 
            complete_command_packet[63],
        ));
        //let command_packet = CommandPacket::from((
        //    complete_command_packet[0],
        //    complete_command_packet[1],
        //    complete_command_packet[2],
        //    complete_command_packet[3..64] 
        //));//let command_packet = CommandPacket::from((
        //    self.pma.read(transaction.endpoint, 0),
        //    self.pma.read(transaction.endpoint, 2),
        //    self.pma.read(transaction.endpoint, 4),
